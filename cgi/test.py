...
log_file_path = os.path.join(log_file_folder, "test.log")
...
parameter_bounds_normalised = torch.zeros(2, num_parameters)#呃呃
...
if (newSolution[0]=="true"):
    ...
    bad_solutions.append(currentSolutions[-1*num_parameters:])#为了不在未来的solution中重复生成已经出现过的solution
    currentSolutions = []
    train_x, train_x_actual = generate_initial_data()
    reply2['train_x_actual'] = train_x_actual.tolist()

    currentSolutions.append(train_x_actual.tolist()[0])#把生成的solution传给html，即rain_x_actual.tolist()[0] 虽然不明白为什么是train_x_actual.tolist()[0]
...
if (nextEvaluation[0]== "true"):
    ...
    cs2 = currentSolutions
    cs = len(currentSolutions)
    ...
    for i in range(int(len(currentSolutions)/num_parameters)):##len(currentSolutions)指的是迄今为止存储的olutions的len，不包括现在在屏幕上显示的这个
    ...
    mll, model = initialize_model(train_x, train_obj)#试图把parameters设计参数和objectives measurements所评估的设计目标 来初始化出一个函数
    ...
    currentSolutions.append(train_x_actual.tolist()[-1])#####所以新的solution其实是一个观测点吗？
    #savedSolutions.append(train_x_actual.tolist()[-1])
    reply2['cs'] = cs
    reply2['cs2'] = cs2
    reply2['solution'] = currentSolutions
