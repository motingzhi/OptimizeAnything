...
<div class="centered-content">
    ...
    <table id="variable-table" class="table">
        <thead>
            <tr>
                <th>Variable</th>
                <th>Range</th>
        ...
        <tbody>
        <!-- Rows will be generated dynamically based on parameterNames.length -->
        </tbody>
    </table>
    <script>
        const parameterNames = [...]; // Assuming this is defined elsewhere in your script
        const parameterBounds = [...]; // Assuming this is defined elsewhere in your script
        
        const tbody = document.querySelector('#variable-table tbody');
        
        parameterNames.forEach((name, index) => {
            const rangeStart = parameterBounds[2 * index];
            const rangeEnd = parameterBounds[2 * index + 1];
            ...
            rangeCell.textContent = `${rangeStart} - ${rangeEnd}`;
            ...
            input.addEventListener('input', () => {
                if (input.value < rangeStart || input.value > rangeEnd) {
        ...
        // Add the logic to store values into savedSolutions and solutionList on form submission
        document.querySelector('form').addEventListener('submit', (e) => {
            ...
            parameterNames.forEach((name, index) => {
                const input = tbody.rows[index].cells[2].querySelector('input').value;
                const rangeStart = parameterBounds[2 * index];
                const rangeEnd = parameterBounds[2 * index + 1];
                
                if (input >= rangeStart && input <= rangeEnd) {
                    savedSolutions.push(input);
                    solutionList.push(input);
                }
            });
